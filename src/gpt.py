...
class MultiheadLatentAttention(nn.Module):
    def __init__(self, config: Config, block_idx: int) -> None:
        ...
        self.q_a_proj = nn.Linear(
            config.n_embd, config.q_lora_rank, bias=config.attn_bias
        )
        self.q_a_norm = RMSNorm(config.q_lora_rank, eps=config.norm_eps)
        self.q_b_proj = nn.Linear(
            config.q_lora_rank, config.n_head * config.qk_head_dim, bias=config.bias
        )
        ...
        # output projection
        self.proj = nn.Linear(
            config.n_head * config.v_head_dim, config.n_embd, bias=config.bias
        )
        # disabled by default
        self.kv_cache: Optional[KVCache] = None
    ...
    def forward(
        ...
        q = self.q_b_proj(
            self.q_a_norm(self.q_a_proj(x))
        )  # (B, T, n_head * qk_head_dim)
        ...
        compressed_kv = self.kv_a_proj_with_mqa(
            x
        )  # (B, T, kv_lora_rank + qk_rope_head_dim)
...
class GroupedTopkRouter(nn.Module):
    ...
    def get_topk_indices(self, scores: torch.Tensor) -> torch.Tensor:
        scores_for_choice = scores.view(
            -1, self.config.n_expert
        ) + self.e_score_correction_bias.unsqueeze(0)
        group_scores = (
            scores_for_choice.view(
                -1,
                self.config.n_expert_groups,
                self.config.n_expert // self.config.n_expert_groups,
            )
            .topk(self.config.n_topk_scores_per_group, dim=-1)[
                0
            ]  # Top k scores for each group
            .sum(dim=-1)
        )

        group_idx = torch.topk(
            group_scores, k=self.config.n_topk_groups, dim=-1, sorted=False
        )[1]
        ...
        topk_indices = torch.topk(
            scores_for_choice, k=self.config.n_expert_per_token, dim=-1, sorted=False
        )[1]
        return topk_indices
