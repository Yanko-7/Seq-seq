...
file_handler = logging.FileHandler("validation_report.log", encoding="utf-8")
...
#         checkpoint_path="checkpoint/nanochat/last.ckpt"
#     )
...
#     success_decode = 0
#     fail_decode = 0
#     pbar = tqdm(range(samples), desc="Generating samples")
#     for i in pbar:
#         tokens = [tokenizer.get_bos_token_id()]
#         ids = [tokenizer.get_bos_token_id()]

#         for token_column, token_masks in engine.generate(
#             tokens, max_tokens=5000, num_samples=1, seed=114514 + i
#         ):
...
#         try:
#             brepdata = tokenizer.decode(ids)
#             viz = PointGridVisualizer()
#         except Exception:
#             fail_decode += 1
#         else:
#             success_decode += 1
#             save_path = f"decodes/sample_{i}.png"
#             viz.visualize(brepdata, save_path=save_path)

#         pbar.set_postfix(
#             {
#                 "Success": success_decode,
#                 "Failed": fail_decode,
#                 "Rate": f"{(success_decode / (i + 1)):.2%}",
#             }
#         )

# print(f"Successfully decoded {success_decode}/{samples} samples")
...
if __name__ == "__main__":
    ...
    model = ForgeTrace.load_from_checkpoint(
        checkpoint_path="checkpoint/nanochat/epoch00-val_loss0.2144.ckpt"
    )
    tokenizer = Tokenizer(checkpoint_dir="weights")
    model = model.to(device="cuda", dtype=torch.bfloat16)
    tokenizer = tokenizer.to(device="cuda")
    ...
    for i in tqdm(range(10)):
        result_tokens = engine.generate_batch(
            tokens, num_samples=64, max_tokens=5000, seed=114514 + i
        )[0]

        # 3. 对当前 batch 的每一行进行 check
        for row_tokens in result_tokens:
            status, msg, error_idx = tokenizer.validate_tokens(row_tokens)
    ...
    # # np.savez_compressed(
    # #     "sample1.npz",
    # #     face_points=brepdata.face_points,
    # #     edge_points=brepdata.edge_points,
    ...
    # viz.visualize(brepdata, save_path=save_path)
    # builder = BRepBuilder(brepdata)
    # shape = builder.build()
    # print(type(shape))
    # save_model_and_image(shape)
